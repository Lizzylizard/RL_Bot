...
class MyImage:
  ...
  # takes now a GRAYSCALE image of shape (1, 50)
  # divides picture into two segments: floor and line
  # sets the background (floor) pixels to black (0)
  # and the line pixels to white (255)
  def segmentation(self, img):
    ...
    # line in received image is black = (0)
    # background in received image is grey = (74)
    # >10 will be set to 0 (black)
    # => background will be set to black.. sorry
    # 0 to 10 will be set to 1 -> 255 (white)
  ...
  def get_line_state(self, img):
    # make 'average' image
    # img = self.make_average_img(img_arr)
  ...
  # makes an 'average' image out of a few consecutive images
  # takes an array of images (1 row = 1 image)
  # adds images up and divides by amount of images
  def make_average_img(self, img_arr):
    print("Shape image 'array' = " + str(np.shape(img_arr)))
    amount = len(img_arr)
    new_img = np.zeros(shape=[1, amount*len(img_arr[0])])
    new_img[0] = img_arr[0]
    ...
    print("Shape average image = " + str(np.shape(new_img)))
    return new_img